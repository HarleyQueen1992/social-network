  ...
  return (
    <div className={h.header}>
      <div className={h.logoBlock}>
        <img
          className={h.logo}
          src='https://png.pngtree.com/png-vector/20200908/ourlarge/pngtree-pug-dog-smoke-pipe-illustration-png-image_2341735.jpg'
        />
        <span className={h.headerName}>
          {" "}
          <NavLink to={"/profile/"}>Mosset</NavLink>{" "}
        </span>
      </div>
      <div
        onClick={() => {
        ...
        className={h.HeaderBurger + " " + (isActive ? h.active : "")}
      >
        <span></span>
      </div>
      ...
      <div className={h.searchBlock}>
        ...
        <input
          value={props.value}
          onDoubleClick={() => {
            props.setWindowMode(false)
          }}
          onChange={props.handleChange}
          onClick={() => {
            props.setWindowMode(true)
          }}
          className={h.searchUsers}
          type='text'
          placeholder='Search..'
        />
      ...
      <div className={h.loginBlock}>
        {props.isAuth ? (
          props.profileInfo === null ? (
            ...
            <div>
              {props.profileInfo.photo === null || undefined ? (
                <div className={h.avaLogin}>
                  <NavLink className={h.ava} to={"/profile/"}>
                  ...
                  <button className={h.logout} onClick={props.logOut}>
                    Log Out
                  </button>
                </div>
              ) : (
                <div className={h.avaLogin}>
                  <NavLink className={h.ava} to={"/profile/"}>
                    <img className={h.ava} src={props.profileInfo.photo} />
                  </NavLink>
                  <span className={h.Name}>
                    {" "}
                    <NavLink to={"/profile/"}>
                  ...
                  {/* <img onClick={props.logOut} className={h.logout} src="https://image.flaticon.com/icons/png/512/25/25706.png"/> */}
                  <NavLink className={h.logOutLink} to={"/login"}>
                    <button className={h.logout} onClick={props.logOut}>
                      Log Out
                    </button>
                  </NavLink>
          ...
          <NavLink to={"/login"} className={h.authBlock}>
            Login
          </NavLink>
        )}
      </div>
      ...
      {props.windowMode && props.value != "" && props.usersSearch != null ? (
        <div className={h.usersSearch}>
          {props.usersSearch.length === 0 ? (
            <div className={h.notFound}>Not found</div>
          ) : (
